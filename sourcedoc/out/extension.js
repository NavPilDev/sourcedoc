...
function detectPaste(change) {
    ...
    if (trimmed.includes('\n'))
        return true;
    if (trimmed.length >= 5 && change.rangeLength === 0)
        return true;
    return false;
}
// =========================
// MODAL POPUP UI
// =========================
async function openAnnotationModal(context, existing, existingRange) {
    return new Promise((resolve) => {
        const lightMode = context.workspaceState.get('sourcedoc.settings.lightMode', false);
        const panel = vscode.window.createWebviewPanel('aiAnnotation', 'AI Annotation', vscode.ViewColumn.Active, {
            enableScripts: true,
...
// MODAL HTML
// =========================
function getModalHtml(existing, existingRange, lightMode) {
...
// SELECTION CHECK
// =========================
function getNonEmptySelection(editor) {
...
function activate(context) {
    const model = new sourcePasteModel_1.SourcePasteModel(context);
    context.subscriptions.push(model);
    const markers = new sourceMarkers_1.SourceMarkers(model);
    context.subscriptions.push(markers);
    const webviewProvider = new sourceWebviewViewProvider_1.SourceWebviewViewProvider(context.extensionUri, model, context, markers);
    context.subscriptions.push(webviewProvider);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider('sourcedoc.sourcePanel', webviewProvider, { webviewOptions: { retainContextWhenHidden: true } }));
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeTextDocument(async (event) => {
        ...
        if (!editor || event.document !== editor.document)
            return;
        const autoDetectEnabled = context.workspaceState.get('sourcedoc.settings.autoAnnotationDetection', true);
        ...
        for (const change of event.contentChanges) {
            ...
            const end = new vscode.Position(change.range.start.line + lines.length - 1, lines.length === 1
    ...
    // EDIT ANNOTATION
    // =========================
    vscode.commands.registerCommand('sourcedoc.annotateExisting', async (annotationId) => {
        if (!annotationId)
            return;
        ...
        if (result.startLine !== undefined && result.endLine !== undefined) {
            model.updateAnnotationRange(annotationId, result.startLine - 1, // convert to 0-based
            result.endLine - 1);
        }
    });
    // =========================
    // DELETE
    // =========================
    context.subscriptions.push(vscode.commands.registerCommand('sourcedoc.deleteAnnotation', async (annotationId) => {
        if (!annotationId)
            return;
    ...
    // EXPORT PDF
    // =========================
    context.subscriptions.push(vscode.commands.registerCommand('sourcedoc.exportPdf', async (args) => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
        ...
        const normalizeChartMode = (v) => {
            return v === 'bar' || v === 'pie' || v === 'donut' || v === 'none' ? v : undefined;
        };
        const toolsChart = normalizeChartMode(exportArgs?.charts?.tools) ??
            context.workspaceState.get('sourcedoc.export.chart.tools', 'donut');
        const modelsChart = normalizeChartMode(exportArgs?.charts?.models) ??
            context.workspaceState.get('sourcedoc.export.chart.models', 'donut');
        const fileLabel = vscode.workspace.asRelativePath(uri);
        const computedDefaultName = (fileLabel.split(/[\\/]/).pop() || 'sourcedoc') + '.pdf';
        const requestedName = typeof exportArgs?.filename === 'string' && exportArgs.filename.trim()
        ...
        if (!saveUri)
            return;
        try {
            await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'SourceDoc: exporting PDF…' }, async () => {
                ...
                function isMeaningfullyChanged(original, current) {
                    return normalizeForDiff(original) !== normalizeForDiff(current);
                }
                function addCodeBlockPageIfNeeded(minHeight) {
                    ensureSpace(minHeight);
                }
                function renderPaginatedCodeBlock(title, code, opts) {
                ...
                function drawBarChart(items, width = 300) {
                    const left = doc.page.margins.left;
                    const rowH = 14;
                    const barH = 8;
                    const labelW = 110;
                ...
                function drawPieLike(items, donut) {
                    const left = doc.page.margins.left;
                    const size = 140;
                    const cx = left + 70;
                    const cy = doc.y + 70;
                ...
                function renderStatsSection(title, mode, items) {
                    const left = doc.page.margins.left;
                    doc.x = left;
                ...
                function renderStatsColumn(x, y, width, title, mode, items) {
                    const left = doc.page.margins.left;
                    const startY = y;
                ...
                for (const [annotationIdx, a] of annotations.entries()) {
                    ...
                    doc.fontSize(10).fillColor('#444444').text(`Lines ${a.range.start.line + 1}–${a.range.end.line + 1} • ${a.recordedAt.toISOString()}`);
                    doc.fillColor('#000000');
                    doc.moveDown(0.2);
